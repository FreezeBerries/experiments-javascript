    ...
    try {
        ...
        if (exisingNameQuery.count > 0) {
            return res.status(409).json({
                "msg": `Conflict: role named '${body.name}' already assigned.`,
...
exports.updateMany = async (req, res) => {
    try {
        ...
        for (var i = 0; i < roles.length; i++) {
            ...
            if (exisingNameQuery.count > 0) {
                return res.status(409).json({
                    "msg": `Conflict: role named '${roles[i].name}' already assigned. Index '${roles[i].id}' cannot use this name current.`
                });
            }
        }
        ...
        for (var i = 0; i < roles.length; i++) {
            ...
            for (var prop in roles[i]) {
                ...
                if (prop == 'id' || prop === 'name' || prop === 'user_id') {
                    continue;
                }
...
exports.updateOne = async (req, res) => {
    try {
        ...
        //insure a valid id
        const id = (await db.query(`SELECT COUNT(*) as doesExist FROM roles WHERE id = :id`, { replacements: { id: req.params.id }, raw: true, type: Sequelize.QueryTypes.SELECT }))[0];

        if (!id.doesExist) {
            return res.status(400).json({
                "msg": `Bad Request: Id '${req.params.id}' does not exist.`
            });
        }

        //prevent names from repeating
        if (body.name) {
            const exisingNameQuery = (await db.query(`SELECT count(*) AS count FROM roles WHERE id = :name`, { replacements: { name: body.name }, raw: true, type: Sequelize.QueryTypes.SELECT }))[0];
            console.log('hit', "exisingNameQuery", exisingNameQuery);
            if (exisingNameQuery.count > 0) {
                return res.status(409).json({
                    "msg": `Conflict: role named '${body.name}' already assigned.`,
                    "humanMsg": "The name for this role is already in use."
        ...
        for (var prop in body) {
            ...
            if(prop !== "name") {
                updateColsReplace.push(body[prop] === "true");
            }
            
            if(prop === "name") {
                updateColsReplace.push(body[prop]);
            }
        }

        if (updateColsSQL.length === 0) {
            return res.status(400).json({
                "msg": `Bad Request: You forgot the body in the request. Nothing to update`
            });
        }

        updateColsReplace.push(req.params.id);
        const updatedRole = await db.query(`UPDATE roles SET ${updateColsSQL.join(', ')} WHERE id = ?`, { replacements: updateColsReplace, raw: true, type: Sequelize.QueryTypes.UPDATE });

        res.status(200).json({ "msg": "Role has been successfully updated." });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ msg: 'Internal server error' });
    }
...
exports.deleteOne = async (req, res) => {
    try {
        ...
        //delete
        const deleteRole = await db.query(`DELETE FROM roles WHERE roles.id = :id`, { replacements: { id: req.params.id }, raw: true, type: Sequelize.QueryTypes.DELETE });

        res.status(200).json({ "msg": "Role has been successfully deleted." });
