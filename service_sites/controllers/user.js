...
exports.postSignup = (req, res, next) => {
  Company.findOneOrCreate((err, companyResult) => {
    var signupcodeValue = signupcode.code;
    if(companyResult.signupcode && companyResult.signupcode !== "") {
      signupcodeValue = companyResult.signupcode;
    }
    req.assert('email', 'Email is not valid').isEmail();
    req.assert('password', 'Password must be at least 4 characters long').len(4);
    req.assert('confirmPassword', 'Passwords do not match').equals(req.body.password);
    req.assert('signupcode', 'Sign Up Code not valid').equals(signupcodeValue);
    req.sanitize('email').normalizeEmail({ gmail_remove_dots: false });
